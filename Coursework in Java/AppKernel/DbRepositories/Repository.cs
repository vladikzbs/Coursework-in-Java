...
namespace Coursework_in_Java.AppKernel.DbRepositories
{
    public class Repository : IRepository
    {
                        ...
                        where TEntity : class
        {
            ...
            DbSet<TEntity> entity = GetEntity<TEntity>();
            TEntity item = entity.Where(where).SingleOrDefault();
        ...
        public DbSet<TEntity> GetItems<TEntity>(Expression<Func<TEntity, bool>> where) where TEntity : class
        {
            ...
            DbSet<TEntity> entity = GetEntity<TEntity>();
            IEnumerable<TEntity> entities = entity.Where(where).ToList();
            DbSet<TEntity> result = entity.Where(where).ToList() as DbSet<TEntity>;
            return result;
        }
